...
namespace Plugin.BetterFirebasePushNotification
{
    ...
    public class FirebasePushNotificationManager : NSObject, IFirebasePushNotification, IUNUserNotificationCenterDelegate, IMessagingDelegate
    {
        ...
        public static void Initialize(NSDictionary options, bool autoRegistration = true)
        {
            ...
            if (options?.ContainsKey(UIApplication.LaunchOptionsRemoteNotificationKey) ?? false)
            {
                ...
                if (pushPayload != null)
                {
                    ...
                    _onNotificationOpened?.Invoke(BetterFirebasePushNotification.Current, new FirebasePushNotificationResponseEventArgs(notificationResponse.Data, notificationResponse.Identifier, notificationResponse.Type));

                    BetterFirebasePushNotification.Current.NotificationHandler?.OnOpened(notificationResponse);
                }
            }
        ...
        public void RegisterForPushNotifications()
        {
            ...
            Messaging.SharedInstance.Delegate = BetterFirebasePushNotification.Current as IMessagingDelegate;
            ...
            if (UIDevice.CurrentDevice.CheckSystemVersion(10, 0))
            {
                ...
                UNUserNotificationCenter.Current.Delegate = BetterFirebasePushNotification.Current as IUNUserNotificationCenterDelegate;

                UNUserNotificationCenter.Current.RequestAuthorization(authOptions, (granted, error) =>
                {
                    if (error != null)
                    {
                        _onNotificationError?.Invoke(BetterFirebasePushNotification.Current, new FirebasePushNotificationErrorEventArgs(FirebasePushNotificationErrorType.PermissionDenied, error.Description));
                    }
                    else if (!granted)
                    {
                        _onNotificationError?.Invoke(BetterFirebasePushNotification.Current, new FirebasePushNotificationErrorEventArgs(FirebasePushNotificationErrorType.PermissionDenied, "Push notification permission not granted"));
                    }
                    else
                    {
        ...
        {
            _onNotificationError?.Invoke(BetterFirebasePushNotification.Current, new FirebasePushNotificationErrorEventArgs(FirebasePushNotificationErrorType.RegistrationFailed, error.Description));
        }
        ...
        public void DidReceiveRegistrationToken(Messaging messaging, string fcmToken)
        {
            ...
            {
                _onTokenRefresh?.Invoke(BetterFirebasePushNotification.Current, new FirebasePushNotificationTokenEventArgs(refreshedToken));
                hasToken = true;
                while (pendingTopics.Count > 0)
                {
                    ...
                    if (pTopic != null)
                    {
                        if (pTopic.Item2)
                        {
                            BetterFirebasePushNotification.Current.Subscribe(pTopic.Item1);
                        }
                        else
                        {
                            BetterFirebasePushNotification.Current.Unsubscribe(pTopic.Item1);
                        }
                    }
                }
            }
